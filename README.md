...
如果你希望把这个 Worker 作为受保护的后端接口给别的系统调用，可以在 Cloudflare Worker 环境变量里设置：
...
const response = await fetch('https://your-worker.workers.dev/v1/audio/speech', {
    ...
    headers: {
        ...
        'Authorization': 'Bearer your-worker-api-key', // 配置了 API_KEY 时必填
    },
    body: JSON.stringify({
        input: "你好，这是一个测试",
...
const response = await fetch('https://your-worker.workers.dev/v1/audio/transcriptions', {
    method: 'POST',
    headers: {
        'x-api-key': 'your-worker-api-key', // 配置了 API_KEY 时必填
    },
    body: formData
});
